...
// Policy storage in Firestore so a purchased policy follows the farmer's account.
app.get("/api/policies", auth, async (req, res) => {
  try {
    const snap = await db.collection("policies")
      ...
      .limit(10)
      .get();
    res.json({ policies: snap.docs.map(doc => doc.data()) });
  } catch (err) {
    // If a Firestore index is not available yet, fall back to a simple query.
    try {
      const snap = await db.collection("policies")
        .where("userId", "==", req.user.id)
        .limit(10)
        .get();
      const policies = snap.docs.map(doc => doc.data())
        .sort((a, b) => String(b.purchasedAt || "").localeCompare(String(a.purchasedAt || "")));
      res.json({ policies });
    } catch (fallbackErr) {
      console.error(fallbackErr);
      res.status(500).json({ error: "Could not load your policies." });
    }
  }
});

app.post("/api/policies/purchase", auth, async (req, res) => {
  const {
    policyId, crop, sumInsured, premium,
    droughtProbability, excessProbability, rainfall30,
    coordinates, state, district, purchasedAt, coverageStart, coverageEnd,
    expectedRainfall
  } = req.body;

  if (!policyId || !crop || !Number.isFinite(Number(sumInsured)) || !Number.isFinite(Number(premium))) {
    return res.status(400).json({ error: "Complete policy details are required before purchase." });
  }
  ...
  const policy = {
    ...
    droughtProbability: Number(droughtProbability || 0),
    excessProbability: Number(excessProbability || 0),
    rainfall30: Number(rainfall30 || 0),
    coordinates: coordinates || null,
    state: state || "",
    district: district || "",
    purchasedAt: purchasedAt || new Date().toISOString(),
    coverageStart: coverageStart || purchasedAt || new Date().toISOString(),
    coverageEnd: coverageEnd || null,
    expectedRainfall: Array.isArray(expectedRainfall) ? expectedRainfall : [],
    status: "Active"
  };
...
app.post("/api/claims", auth, async (req, res) => {
  ...
  if (!policyId) return res.status(400).json({ error: "Please purchase an active insurance policy first." });
  ...
  const policyDoc = await db.collection("policies").doc(String(policyId)).get();
  if (!policyDoc.exists || policyDoc.data().userId !== req.user.id || policyDoc.data().status !== "Active") {
    return res.status(400).json({ error: "The selected insurance policy is not active." });
  }

  const policy = policyDoc.data();
  const claimAmount = Number(amount);
  if (!Number.isFinite(claimAmount) || claimAmount <= 0) {
    return res.status(400).json({ error: "Enter a valid claim amount." });
  }
  if (claimAmount > Number(policy.sumInsured)) {
    return res.status(400).json({ error: "Claim amount cannot exceed the sum insured." });
  }

  // Demo sensor reading requested for the hackathon claim screen.
  const sensorRainfallMm = 0;
  const expected = Number(policy.expectedRainfall?.[0]?.expectedMm || 0);
  const droughtTrigger = expected > 0 && sensorRainfallMm < expected * 0.70;
  const excessTrigger = expected > 0 && sensorRainfallMm > expected * 1.30;
  ...
  const claim = {
    ...
    trigger: droughtTrigger ? "Drought trigger (R < 0.7E)" : excessTrigger ? "Excess rainfall trigger (R > 1.3E)" : "Manual review",
    status: "Submitted",
