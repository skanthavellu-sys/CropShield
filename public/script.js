...
$("savePolicyBtn").addEventListener("click",()=>{if(!latestRainfall)return;;const pDrought=droughtProbability(latestRainfall.historical.totalMm,crop);const pLoss=cropLossProbability[crop]||0.40;const insuredAmount=sum;const premium=pDrought*insuredAmount*1.20;currentPolicy={id:"CS-"+Date.now().toString().slice(-8),crop,sum,coverage,insuredAmount,premium,droughtProbability:pDrought,lossProbability:pLoss,rainfall30:latestRainfall.historical.totalMm,coordinates:latestRainfall.coordinates,state:$("calcState").value,district:$("calcDistrict").value,createdAt:new Date().toISOString()};localStorage.setItem("cropshield_policy",JSON.stringify(currentPolicy));renderPolicy();toast("Prototype policy saved.");navigate("policy")});
